...
<template>
  ...
  <Transition name="modal-animation">
    <div v-if="isOpen" class="fixed inset-0 flex justify-center items-center z-49">
      ...
      <div
        class="relative rounded-md shadow-md bg-white z-50 p-3 w-full max-w-md max-h-[450px] overflow-y-auto flex flex-col gap-3 mx-3">
        ...
        <div v-if="AIResponse" class="flex w-full">
          <button class="text-white rounded-xl font-semibold p-2 bg-[#ee2800] cursor-pointer ml-auto"
            @click="$emit('close', false)">Zatvori</button>
        </div>
      </div>
